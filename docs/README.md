...
| [TRANSFORM-PAIRING.md](TRANSFORM-PAIRING.md) | **Interpolation that pairs the right cameras and objects.** How RT64 pairs each frame's cameras and objects with the last frame's; the two-player race-start burst (split-screen views paired with each other's cameras) and objects paired across the course, each measured and captured; the two fixes in RT64; and the tools that found them -- a pairing log, a window capture that ignores what covers the game, and a two-player input script. A manual and a recipe for another port. |
